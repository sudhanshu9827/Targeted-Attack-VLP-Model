...
class ImageAttacker():
    ...
    def txt_guided_attack(self, model, imgs, img2txt, txt2img, device, scales=None, txt_embeds=None,
        ...
        with torch.enable_grad():
            ...
            chunk_size = 32  # process 32 images at a time
            adv_imgs_list = []
            imgs_list = []
            
            for chunk_start in range(0, adv_imgs.shape[0], chunk_size):
                chunk_end = min(chunk_start + chunk_size, adv_imgs.shape[0])
                adv_chunk = adv_imgs[chunk_start:chunk_end]
                img_chunk = scaled_imgs[chunk_start:chunk_end]
                
                if self.normalization is not None:
                    adv_out = model.inference_image(self.normalization(adv_chunk))
                    img_out = model.inference_image(self.normalization(img_chunk))
                else:
                    adv_out = model.inference_image(adv_chunk)
                    img_out = model.inference_image(img_chunk)
                
                adv_imgs_list.append(adv_out['image_feat'])
                imgs_list.append(img_out['image_feat'])
            
            adv_imgs_output = {'image_feat': torch.cat(adv_imgs_list, dim=0)}
            imgs_output = {'image_feat': torch.cat(imgs_list, dim=0)}
